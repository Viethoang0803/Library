...
//            btnUpdate.setVisible(true);
    ...
    public void onClickAdd(ActionEvent actionEvent) {
        ...
        if (isNull(bookId, bookName, category, quantity, publishDate, author)) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "Lỗi", null, "Hãy điền đẩy đủ các trường thông tin!");
            return;
        }

        // validate
        if (!isValid("INTEGER_NUMBER", quantity)) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "Lỗi", null, "Số lượng phải là một số nguyên!");
            return;
        }
        ...
        try {
            ...
            AlertUtil.showAlert(Alert.AlertType.INFORMATION, "Thông báo", null, "Thêm sách mới thành công!");
            tbBooks.setItems(bookService.getAllBook());
            clear();
    ...
    public void onClickDelete(ActionEvent actionEvent) {
        ...
        if (selectedBook.isPresent() && AlertUtil.showConfirmation("Bạn có chắc chắn muốn xóa sách?\nViệc này không thể hoàn tác!")) {
            bookService.deleteBook(selectedBook.get());
        } else if (selectedBook.isEmpty()) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "Lỗi", null, "Hãy chọn sách muốn xóa!");
        }
        tbBooks.setItems(bookService.getAllBook());
    }
    ...
    public void onClickUpdate(ActionEvent actionEvent) {
        ...
        //check null
        if (isNull(bookId, bookName, category, quantity, publishDate, author)) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "Lỗi", null, "Hãy điền đầy đủ các trường thông tin!");
            return;
        }
        ...
        if (selectedBook.isPresent()) {
            ...
            tbBooks.setItems(bookService.getAllBook());
            AlertUtil.showAlert(Alert.AlertType.INFORMATION, "Thành công", null, "Cập nhật sách thành công!");
            clear();
        } else {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "Lỗi", null, "Hãy chọn sách để cập nhật!");
        }
...
//        btnUpdate.setVisible(false);
    ...
    public void onClickRequest(ActionEvent actionEvent) {
        ...
        if (selectedBook.getQuantity() == 0) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "Lỗi", null, "Quyển sách này đã hết!");
            return;
        }
        ...
        if (returnDate == null) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "Lỗi", null, "Hãy chọn ngày hẹn trả!");
            return;
        ...
        AlertUtil.showAlert(Alert.AlertType.INFORMATION, "Thành công", null, "Yêu cầu mượn thành công!");
